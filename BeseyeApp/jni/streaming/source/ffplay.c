...
const int program_birth_year2 = 2003;
...
//yehudi static int sws_flags = SWS_BICUBIC;

 typedef struct PacketQueue2 { //yehudi
...
#if 0 //yehudi
typedef struct VideoPicture { //yehudi
...
#endif //yehudi
typedef struct SubPicture2 {//yehudi
    double pts; /* presentation time stamp for this picture */
    AVSubtitle sub;
} SubPicture2; //yehudi

typedef struct AudioParams2 { //yehudi
    int freq;
    int channels;
    int channel_layout;
...
//yehudi enum {
//yehudi     AV_SYNC_AUDIO_MASTER, /* default choice */
//yehudi     AV_SYNC_VIDEO_MASTER,
//yehudi    AV_SYNC_EXTERNAL_CLOCK, /* synchronize to an external clock */
//yehudi };
#if 0 //yehudi
typedef struct VideoState2 { //yehudi
    SDL_Thread *read_tid;
...
#endif //yehudi
typedef struct AllocEventProps2 { //yehudi
    VideoState *is;
    AVFrame *frame;
} AllocEventProps2; //yehudi
...
void av_noreturn exit_program2(int ret)  //yehudi
{
    exit(ret);
}
...
static int packet_queue_put2(PacketQueue *q, AVPacket *pkt)  //yehudi
{
    int ret;
...
static void packet_queue_init2(PacketQueue *q) //yehudi
{
...
static void packet_queue_flush2(PacketQueue *q) //yehudi
{
...
static void packet_queue_destroy2(PacketQueue *q) //yehudi
{
...
static void packet_queue_abort2(PacketQueue *q) //yehudi
{
    SDL_LockMutex(q->mutex);
...
static void packet_queue_start2(PacketQueue *q) //yehudi
{
    SDL_LockMutex(q->mutex);
    q->abort_request = 0;
...
static int packet_queue_get2(PacketQueue *q, AVPacket *pkt, int block) //yehudi
{
    AVPacketList *pkt1;
    int ret;
...
static void video_audio_display(VideoState *s)
{
    ...
    if (s->show_mode == SHOW_MODE_WAVES) {
        ...
        //yehudi SDL_UpdateRect(screen, s->xleft, s->ytop, s->width, s->height);
    } else {
        ...
        //yehudi SDL_UpdateRect(screen, s->xpos, s->ytop, 1, s->height);
        if (!s->paused)
            s->xpos++;
        if (s->xpos >= s->width)
            s->xpos= s->xleft;
...
static void alloc_picture(AllocEventProps *event_props)
{
    ...
    //yehudi vp->bmp = SDL_CreateYUVOverlay(vp->width, vp->height,
    //yehudi                                SDL_YV12_OVERLAY,
    //yehudi                                screen);
    if (0) {//yehudi (!vp->bmp || vp->bmp->pitches[0] < vp->width) {
...
static int queue_picture(VideoState *is, AVFrame *src_frame, double pts1, int64_t pos)
{
    ...
    if (/*!vp->bmp || yehudi*/ vp->reallocate ||
        ...
        if (0) {//yehudi (is->videoq.abort_request && SDL_PeepEvents(&event, 1, SDL_GETEVENT, SDL_EVENTMASK(FF_ALLOC_EVENT)) != 1) {
            while (!vp->allocated) {
                SDL_CondWait(is->pictq_cond, is->pictq_mutex);
            }
        }
    ...
    if (vp->bmp) {
        ...
        //yehudi pict.data[0] = vp->bmp->pixels[0];
        //yehudi pict.data[1] = vp->bmp->pixels[2];
        //yehudi pict.data[2] = vp->bmp->pixels[1];

        //yehudi pict.linesize[0] = vp->bmp->pitches[0];
        //yehudi pict.linesize[1] = vp->bmp->pitches[2];
        //yehudi pict.linesize[2] = vp->bmp->pitches[1];
...
static int stream_component_open(VideoState *is, int stream_index)
{
    ...
    case AVMEDIA_TYPE_VIDEO:
        ...
        //yehudi is->video_tid = SDL_CreateThread(video_thread, is);
        break;
    case AVMEDIA_TYPE_SUBTITLE:
        ...
        //yehudi         is->subtitle_tid = SDL_CreateThread(subtitle_thread, is);
        break;
    default:
        break;
    }
...
static void stream_component_close(VideoState *is, int stream_index)
{
    ...
    avcodec_close(avctx);
#if CONFIG_AVFILTER
    //yehudi free_buffer_pool(&is->buffer_pool);
#endif
    switch (avctx->codec_type) {
    case AVMEDIA_TYPE_AUDIO:
        is->audio_st = NULL;
...
static int read_thread(void *arg)
{
    ...
    //yehudi     is->refresh_tid = SDL_CreateThread(refresh_thread, is);
...
static VideoState *stream_open(const char *filename, AVInputFormat *iformat)
{
    ...
    //yehudi is->read_tid     = SDL_CreateThread(read_thread, is);
    if (!is->read_tid) {
        av_free(is);
        return NULL;
    }
...
static void toggle_audio_display(VideoState *is)
{
    ...
    fill_rectangle(screen,
                ...
                bgcolor);
    //yehudi SDL_UpdateRect(screen, is->xleft, is->ytop, is->width, is->height);
}
...
int main13(int argc, char **argv)
{
    ...
    //yehudi if (display_disable)
        //yehudi SDL_putenv(dummy_videodriver); /* For the event queue, we always need a video driver. */
#if 0 //yehudi !defined(__MINGW32__) && !defined(__APPLE__)
    ...
    //yehudi SDL_EventState(SDL_ACTIVEEVENT, SDL_IGNORE);
