...
static void(*avfcCb)(AVFormatContext *, void*) = NULL;
static void(*rtmpStatusCb)(void* , int , void*) = NULL;
static void* userdataCb = NULL;
//Abner End
...
static FilterGraph *init_simple_filtergraph(InputStream *ist, OutputStream *ost)
{
    ...
    av_log(NULL, AV_LOG_DEBUG,"init_simple_filtergraph(), fg->nb_outputs:%d", fg->nb_outputs);
    ...
    av_log(NULL, AV_LOG_DEBUG,"init_simple_filtergraph(), fg->nb_inputs:%d", fg->nb_inputs);
    ...
    av_log(NULL, AV_LOG_DEBUG,"init_simple_filtergraph(), ist->nb_filters:%d", ist->nb_filters);

    ist->filters = grow_array(ist->filters, sizeof(*ist->filters),
                              &ist->nb_filters, ist->nb_filters + 1);
    ist->filters[ist->nb_filters - 1] = fg->inputs[0];

    av_log(NULL, AV_LOG_DEBUG,"init_simple_filtergraph(), %d, %d, ist->nb_filters:%d", fg->nb_outputs, fg->nb_inputs, ist->nb_filters);

    av_log(NULL, AV_LOG_DEBUG,"init_simple_filtergraph(), grow_array, nb_filtergraphs:%d", nb_filtergraphs);
...
static int graph_check_validity(AVFilterGraph *graph, AVClass *log_ctx)
{
	...
	int i, j;
	av_log(NULL, AV_LOG_DEBUG,"graph_check_validity()+\n");
	for (i = 0; i < graph->filter_count; i++) {
		...
		av_log(NULL, AV_LOG_DEBUG,"graph_check_validity(), filt->nb_outputs;:%d", filt->nb_outputs);
		for (j = 0; j < filt->nb_outputs; j++) {
			av_log(NULL, AV_LOG_DEBUG,"graph_check_validity(), filt->outputs[j]:%s", filt->outputs[j] );
...
{
	av_log(NULL, AV_LOG_DEBUG,"configure_filtergraph()+\n");
    ...
    av_log(NULL, AV_LOG_DEBUG,"configure_filtergraph(), 1, simple:%d", simple);
    if (simple) {
    ...
    av_log(NULL, AV_LOG_DEBUG,"configure_filtergraph(), 2, fg->graph %d, graph_desc %s", fg->graph, graph_desc);
    if ((ret = avfilter_graph_parse2(fg->graph, graph_desc, &inputs, &outputs)) < 0)
        return ret;

    av_log(NULL, AV_LOG_DEBUG,"configure_filtergraph(), 3\n");
    ...
    av_log(NULL, AV_LOG_DEBUG,"configure_filtergraph(), 4\n");
    for (cur = inputs, i = 0; cur; cur = cur->next, i++)
        if ((ret = configure_input_filter(fg, fg->inputs[i], cur)) < 0)
            return ret;
    avfilter_inout_free(&inputs);

    av_log(NULL, AV_LOG_DEBUG,"configure_filtergraph(), 5\n");
    if (!init || simple) {
...
//        av_log(NULL, AV_LOG_ERROR,"configure_filtergraph(), 6\n");
//        graph_check_validity(fg->graph, NULL);
...
static int configure_complex_filters(void)
{
    ...
    av_log(NULL, AV_LOG_DEBUG,"configure_complex_filters()+, nb_filtergraphs:%d", nb_filtergraphs);
    for (i = 0; i < nb_filtergraphs; i++){
    	 av_log(NULL, AV_LOG_DEBUG,"configure_complex_filters()+, filtergraphs%d", filtergraphs);
...
void deinit(int ret)
{
	av_log(NULL, AV_LOG_INFO, "deinit(),+\n");
    int i, j;

    av_log(NULL, AV_LOG_DEBUG, "deinit(), nb_filtergraphs:%d", nb_filtergraphs);
    for (i = 0; i < nb_filtergraphs; i++) {
    ...
    av_log(NULL, AV_LOG_DEBUG, "deinit(), nb_output_files:%d", nb_output_files);
    /* close files */
    for (i = 0; i < nb_output_files; i++) {

        AVFormatContext *s = output_files[i]->ctx;
        if (!(s->oformat->flags & AVFMT_NOFILE) && s->pb){
        	av_log(NULL, AV_LOG_ERROR, "exit_program(), avio_close invoked\n");
            avio_close(s->pb);
        }
...
{
	av_log(NULL, AV_LOG_DEBUG, "exit_program(),+\n");
//    int i, j;
//
//    for (i = 0; i < nb_filtergraphs; i++) {
//        avfilter_graph_free(&filtergraphs[i]->graph);
...
//    av_freep(&subtitle_out);
//
//    av_log(NULL, AV_LOG_DEBUG, "exit_program(), nb_output_files:%d", nb_output_files);
//    /* close files */
//    for (i = 0; i < nb_output_files; i++) {
//        AVFormatContext *s = output_files[i]->ctx;
//        if (!(s->oformat->flags & AVFMT_NOFILE) && s->pb){
...
//    }
//
//    av_log(NULL, AV_LOG_DEBUG, "exit_program(), nb_output_streams:%d", nb_output_streams);
//    for (i = 0; i < nb_output_streams; i++) {
//        AVBitStreamFilterContext *bsfc = output_streams[i]->bitstream_filters;
//        while (bsfc) {
//            AVBitStreamFilterContext *next = bsfc->next;
...
//    }
//
//    av_log(NULL, AV_LOG_DEBUG, "exit_program(), nb_input_files:%d", nb_input_files);
//    for (i = 0; i < nb_input_files; i++) {
//        avformat_close_input(&input_files[i]->ctx);
//        av_freep(&input_files[i]);
//    }
//
//    av_log(NULL, AV_LOG_DEBUG, "exit_program(), nb_input_streams:%d", nb_input_streams);
//    for (i = 0; i < nb_input_streams; i++) {
//        av_freep(&input_streams[i]->decoded_frame);
//        av_dict_free(&input_streams[i]->opts);
//        free_buffer_pool(&input_streams[i]->buffer_pool);
...
static int transcode_init(void)
{
    ...
    av_log(NULL, AV_LOG_DEBUG,"transcode_init(), nb_input_files:%d", nb_input_files);
    ...
    av_log(NULL, AV_LOG_DEBUG,"transcode_init(), nb_output_files:%d", nb_output_files);
    /* output stream init */
    for (i = 0; i < nb_output_files; i++) {
    ...
    av_log(NULL, AV_LOG_DEBUG,"transcode_init(), nb_filtergraphs:%d", nb_filtergraphs);
    /* init complex filtergraphs */
    for (i = 0; i < nb_filtergraphs; i++)
        if ((ret = avfilter_graph_config(filtergraphs[i]->graph, NULL)) < 0)
            return ret;

    av_log(NULL, AV_LOG_DEBUG,"transcode_init(), nb_output_streams:%d", nb_output_streams);
    ...
    for (i = 0; i < nb_output_streams; i++) {
        ...
        if (ost->stream_copy) {
            ...
            if (!ost->filter &&
                (codec->codec_type == AVMEDIA_TYPE_VIDEO ||
                 codec->codec_type == AVMEDIA_TYPE_AUDIO)) {
                    FilterGraph *fg;
                    av_log(NULL, AV_LOG_DEBUG,"transcode_init(), init_simple_filtergraph\n");
            ...
            switch (codec->codec_type) {
            case AVMEDIA_TYPE_AUDIO:
            	av_log(NULL, AV_LOG_DEBUG,"transcode_init(), ost->filter->filter->inputs[0]:%d", ost->filter->filter->inputs[0]);
...
static int transcode(void)
{
    ...
    if (!(no_packet = av_mallocz(nb_input_files)))
        exit_program(1);
    av_log(NULL, AV_LOG_DEBUG,"transcode(), transcode_init in\n");
    ...
    av_log(NULL, AV_LOG_DEBUG,"transcode(), begin loop\n");
    for (; received_sigterm == 0;) {
        ...
        //av_log(NULL, AV_LOG_DEBUG,"transcode()+, ret:%d", ret);
...
static int opt_input_file(OptionsContext *o, const char *opt, const char *filename)
{
    ...
    if(rtmpStatusCb){
    	rtmpStatusCb(userdataCb, STREAM_CONNECTING , NULL);
    }
...
static void opt_output_file(void *optctx, const char *filename)
{
    ...
    av_log(NULL, AV_LOG_DEBUG, "opt_output_file()0\n");
    if (!strcmp(filename, "-"))
        filename = "pipe:";

    av_log(NULL, AV_LOG_DEBUG, "opt_output_file()1\n");
    ...
    if(avfcCb && 0 < nb_output_files && NULL != output_files && NULL != output_files[0]) {
		AVFormatContext* oc = output_files[0]->ctx;
		if (oc){
			avfcCb(oc, userdataCb);
		}
	}
    ...
    av_log(NULL, AV_LOG_DEBUG, "opt_output_file()2\n");
    ...
    if (!strcmp(file_oformat->name, "ffm") &&
    	...
    	av_log(NULL, AV_LOG_DEBUG, "opt_output_file()3-1\n");
        int j;
    	...
    	av_log(NULL, AV_LOG_DEBUG, "opt_output_file()3-2\n");
        ...
        av_log(NULL, AV_LOG_DEBUG, "opt_output_file()3-2-1\n");
        ...
        av_log(NULL, AV_LOG_DEBUG, "opt_output_file()3-2-2, o->subtitle_disable:%d, oc->oformat->subtitle_codec:%d ", o->subtitle_disable, oc->oformat->subtitle_codec);
        ...
        av_log(NULL, AV_LOG_DEBUG, "opt_output_file()3-2-3\n");
        /* do something with data? */
    } else {
    	av_log(NULL, AV_LOG_DEBUG, "opt_output_file()3-3\n");
        for (i = 0; i < o->nb_stream_maps; i++) {
...
loop_end:
    ...
    av_log(NULL, AV_LOG_DEBUG, "opt_output_file()4\n");
    ...
    av_log(NULL, AV_LOG_DEBUG, "opt_output_file()5\n");
    ...
    av_log(NULL, AV_LOG_DEBUG, "opt_output_file()6\n");
    ...
    av_log(NULL, AV_LOG_DEBUG, "opt_output_file()7\n");
    ...
    av_log(NULL, AV_LOG_DEBUG, "opt_output_file()8\n");
    if (!(oc->oformat->flags & AVFMT_NOFILE)) {
        /* test if it already exists to avoid losing precious files */
        assert_file_overwrite(filename);

        av_log(NULL, AV_LOG_DEBUG, "opt_output_file()8-1, oc->pb:%d, oc->interrupt_callback:%d, nb_output_files:%d", oc->pb, oc->interrupt_callback, nb_output_files);
        /* open the file */
        if ((err = avio_open2(&oc->pb, filename, AVIO_FLAG_WRITE,
            ...
            print_error(filename, err);
            exit_program(1);
            av_log(NULL, AV_LOG_WARNING, "opt_output_file()8,err:%d",err);
            if(rtmpStatusCb){
                rtmpStatusCb(userdataCb, STREAM_CONNECTED , NULL);
            }
        }
    }
    ...
    av_log(NULL, AV_LOG_ERROR, "opt_output_file()9\n");
    if (o->mux_preload) {
        uint8_t buf[64];
    ...
    av_log(NULL, AV_LOG_DEBUG, "opt_output_file()10\n");
    /* copy metadata */
    ...
    av_log(NULL, AV_LOG_DEBUG, "opt_output_file()11\n");
////    /* copy chapters */
...
////        }
////    }
//    av_log(NULL, AV_LOG_DEBUG, "opt_output_file()12\n");
//    if (o->chapters_input_file >= 0)
//        copy_chapters(input_files[o->chapters_input_file], output_files[nb_output_files - 1],
//                      !o->metadata_chapters_manual);
//
//    av_log(NULL, AV_LOG_DEBUG, "opt_output_file()13\n");
...
//    }
//
//    av_log(NULL, AV_LOG_DEBUG, "opt_output_file()14\n");
...
//        }
//
//    av_log(NULL, AV_LOG_DEBUG, "opt_output_file()15\n");
...
//    }
    ...
    reset_options(o, 0);
    av_log(NULL, AV_LOG_ERROR, "opt_output_file()-\n");
}
...
{
	av_log(NULL, AV_LOG_INFO,"opt_filter_complex()+, nb_filtergraphs:%d", nb_filtergraphs);
    filtergraphs = grow_array(filtergraphs, sizeof(*filtergraphs),
                              &nb_filtergraphs, nb_filtergraphs + 1);
    av_log(NULL, AV_LOG_INFO,"opt_filter_complex(), nb_filtergraphs:%d", nb_filtergraphs);
...
int runAudioStreaming(int argc, char **argv, void(* cb)(AVFormatContext *, void*), void (*rtmpStatusCallback)(void* , int , void*), void* userdata){
	av_log(NULL, AV_LOG_INFO,"runAudioStreaming()+\n");
    OptionsContext o = { 0 };
    int64_t ti;
    exit_code = 0;
    ...
    avfcCb = cb;
    rtmpStatusCb = rtmpStatusCallback;
    userdataCb = userdata;

    /* parse options */
    av_log(NULL, AV_LOG_INFO,"runAudioStreaming(), begin to parse_options\n");
    parse_options(&o, argc, argv, options, opt_output_file);

    av_log(NULL, AV_LOG_INFO,"runAudioStreaming(), end to parse_options\n");
    if (!exit_code && nb_output_files <= 0 && nb_input_files == 0) {
        show_usage();
        av_log(NULL, AV_LOG_WARNING, "Use -h to get full help or, even better, run 'man %s'\n", program_name);
        exit_program(1);
    }

    av_log(NULL, AV_LOG_INFO,"runAudioStreaming(), 1\n");
    /* file converter / grab */
    if (!exit_code && nb_output_files <= 0) {
        av_log(NULL, AV_LOG_FATAL, "At least one output file must be specified\n");
        exit_program(1);
        av_log(NULL, AV_LOG_INFO,"runAudioStreaming(), 1-\n");
    }

    av_log(NULL, AV_LOG_INFO,"runAudioStreaming(), 2\n");
    if (!exit_code && nb_input_files == 0) {
        av_log(NULL, AV_LOG_FATAL, "At least one input file must be specified\n");
        exit_program(1);
    }

    av_log(NULL, AV_LOG_INFO,"runAudioStreaming(), begin to transcode\n");
    current_time = ti = getutime();
    if (!exit_code && transcode() < 0)
        exit_program(1);

    av_log(NULL, AV_LOG_INFO,"runAudioStreaming(), end to transcode\n");
//    ti = getutime() - ti;
//    if (do_benchmark) {
//        int maxrss = getmaxrss() / 1024;
//        printf("bench: utime=%0.3fs maxrss=%ikB\n", ti / 1000000.0, maxrss);
