...
//    beseyeCBFuncHolder* holder = (beseyeCBFuncHolder*)malloc(sizeof(beseyeCBFuncHolder));
//    
//    holder.value = "beseye";

    
    beseyeCBFuncHolder holder;// = {"beseye", rtmpStreamMethodCallback, rtmpStatusCallback, rtmpErrorCallback, (CBeseyeRTMPObserver*)player};
    holder.value = (const char*)"beseye";
    ...
    holder.userData = (CBeseyeRTMPObserver*)player;
    av_log(NULL, AV_LOG_ERROR, "avformat_open_input(), holder:%x, holder.value:%x, rtmpStreamMethodCallback:%x, pos:%x, value:%x, holder.userData:%x, player:%x\n", &holder, &holder.value, rtmpStreamMethodCallback, &holder.rtmpCallback, holder.rtmpCallback, holder.userData,  player);

//    int rrr= av_dict_set(&format_opts, "holder", (const char*)&holder, AV_DICT_DONT_STRDUP_VAL);
//
//    av_log(NULL, AV_LOG_ERROR, "rrr:%d\n",rrr);
//    AVDictionaryEntry *tt = av_dict_get(format_opts, "holder", NULL, 0);
//    
//    if(NULL != tt && NULL != tt->value){
//    	av_log(NULL, AV_LOG_ERROR, "check holder value:%x\n", tt->value);
//        beseyeCBFuncHolder* holder2 = (beseyeCBFuncHolder*)tt->value;
// av_log(NULL, AV_LOG_ERROR, "avformat_open_input(), holder2:%x, holder2.value:%x, rtmpStreamMethodCallback:%x, pos:%x, value:%x, holder2.userData:%x, player:%x\n", holder2, holder2->value, rtmpStreamMethodCallback, holder2->rtmpCallback, holder2->rtmpCallback, holder2->userData,  player);
//    }
