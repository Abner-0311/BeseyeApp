                          ...
                          unsigned int offset, unsigned int max_probe_size)
{
    ...
    av_log(0, AV_LOG_ERROR, "av_probe_input_buffer 0\n");
    if (!max_probe_size) {
    ...
    for(probe_size= PROBE_BUF_MIN; probe_size<=max_probe_size && !*fmt;
    	...
    	av_log(0, AV_LOG_ERROR, "av_probe_input_buffer 1\n");
        ...
        buf=buftmp;
        av_log(0, AV_LOG_ERROR, "av_probe_input_buffer 1-1\n");
        ...
        /* guess file format */
        av_log(0, AV_LOG_ERROR, "av_probe_input_buffer 1-2\n");
    ...
    av_log(0, AV_LOG_ERROR, "av_probe_input_buffer 2\n");
    if (!*fmt) {
        av_free(buf);
        return AVERROR_INVALIDDATA;
    }

    av_log(0, AV_LOG_ERROR, "av_probe_input_buffer 3\n");
...
static int init_input(AVFormatContext *s, const char *filename, AVDictionary **options)
{
    ...
    av_log(0, AV_LOG_ERROR, "init_input 0\n");
    if (s->pb) {
    	av_log(0, AV_LOG_ERROR, "init_input 1\n");
    ...
    av_log(0, AV_LOG_ERROR, "init_input 2\n");
    if ( (s->iformat && s->iformat->flags & AVFMT_NOFILE) ||
        (!s->iformat && (s->iformat = av_probe_input_format(&pd, 0))))
        return 0;

    av_log(0, AV_LOG_ERROR, "init_input 3\n");
...
int avformat_open_input(AVFormatContext **ps, const char *filename, AVInputFormat *fmt, AVDictionary **options)
{
    ...
    av_log(0, AV_LOG_ERROR, "avformat_open_input 0\n");
...
//    }
    ...
    av_log(0, AV_LOG_ERROR, "avformat_open_input 2\n");
...
//    }
    ...
    av_log(0, AV_LOG_ERROR, "avformat_open_input 3\n");
    if ((ret = init_input(s, filename, &tmp)) < 0)
        goto fail;

    av_log(0, AV_LOG_ERROR, "avformat_open_input 4\n");
    ...
    av_log(0, AV_LOG_ERROR, "avformat_open_input 5\n");
    s->duration = s->start_time = AV_NOPTS_VALUE;
    av_strlcpy(s->filename, filename ? filename : "", sizeof(s->filename));

    av_log(0, AV_LOG_ERROR, "avformat_open_input 6\n");
    ...
    if (s->iformat->priv_data_size > 0) {
        ...
        av_log(0, AV_LOG_ERROR, "avformat_open_input 6-1\n");
    ...
    av_log(0, AV_LOG_ERROR, "avformat_open_input 7\n");
    /* e.g. AVFMT_NOFILE formats will not have a AVIOContext */
    if (s->pb)
        ff_id3v2_read(s, ID3v2_DEFAULT_MAGIC, &id3v2_extra_meta);

    av_log(0, AV_LOG_ERROR, "avformat_open_input 8\n");
    if (!(s->flags&AVFMT_FLAG_PRIV_OPT) && s->iformat->read_header)
        if ((ret = s->iformat->read_header(s)) < 0)
            goto fail;

    av_log(0, AV_LOG_ERROR, "avformat_open_input 9\n");
    if (id3v2_extra_meta &&
        (ret = ff_id3v2_parse_apic(s, &id3v2_extra_meta)) < 0)
        goto fail;
    av_log(0, AV_LOG_ERROR, "avformat_open_input 10\n");
    ff_id3v2_free_extra_meta(&id3v2_extra_meta);

    av_log(0, AV_LOG_ERROR, "avformat_open_input 11\n");
    queue_attached_pictures(s);

    av_log(0, AV_LOG_ERROR, "avformat_open_input 12\n");
...
/*            if(!st->time_base.num)
        ...
        if (!st->parser && !(ic->flags & AVFMT_FLAG_NOPARSE)) {
            ...
            if(st->parser){
                ...
                av_log(NULL, AV_LOG_VERBOSE, "parser not found for codec "
                       "%s, packets or times may be invalid.\n",
                       avcodec_get_name(st->codec->codec_id));
            }
        }
        ...
        av_log(NULL, AV_LOG_INFO, "1-1, %d: %d\n", i, (codec != NULL)?codec->id:0);
    }
    ...
    for(;;) {
        ...
        if (i == ic->nb_streams) {
            ...
            if (!(ic->ctx_flags & AVFMTCTX_NOHEADER)) {
                ...
                ret = count;
                av_log(NULL, AV_LOG_DEBUG, "All info found\n");
                flush_codecs = 0;
                break;
            }
        }

        //av_log(NULL, AV_LOG_INFO, "3-3, 0: %d\n", (ic->streams[0] && ic->streams[0]->codec != NULL)?ic->streams[0]->codec->codec_id:0);
        /* we did not get all the codec info, but we read too much data */
        if (read_size >= ic->probesize) {
            ret = count;
            av_log(NULL, AV_LOG_DEBUG, "Probe buffer size limit %d reached\n", ic->probesize);
            for (i = 0; i < ic->nb_streams; i++)
        ...
        //av_log(NULL, AV_LOG_INFO, "3-4, 0: %d\n", (ic->streams[0] && ic->streams[0]->codec != NULL)?ic->streams[0]->codec->codec_id:0);
        ...
        //av_log(NULL, AV_LOG_INFO, "3-5, 0: %d\n", (ic->streams[0] && ic->streams[0]->codec != NULL)?ic->streams[0]->codec->codec_id:0);
        ...
        //av_log(NULL, AV_LOG_INFO, "3-6, 0: %d\n", (ic->streams[0] && ic->streams[0]->codec != NULL)?ic->streams[0]->codec->codec_id:0);
        st = ic->streams[pkt->stream_index];
        if (st->codec_info_nb_frames>1) {
            ...
            if (t >= ic->max_analyze_duration) {
                av_log(NULL, AV_LOG_WARNING, "max_analyze_duration %d reached at %"PRId64"\n", ic->max_analyze_duration, t);
                break;
            }
        ...
        }
        {
            ...
            av_log(NULL, AV_LOG_INFO, "3-7, 0: %d", (ic->streams[0]->codec != NULL)?ic->streams[0]->codec->codec_id:0);
...
//                     av_log(NULL, AV_LOG_ERROR, "%f\n", dts);
        ...
        //av_log(NULL, AV_LOG_INFO, "3-8, 0: %d", (ic->streams[0]->codec != NULL)?ic->streams[0]->codec->codec_id:0);
        if(st->parser && st->parser->parser->split && !st->codec->extradata){
    ...
    if (flush_codecs) {
        ...
        av_log(NULL, AV_LOG_INFO, "4-1, 0: %d", (ic->streams[0] && ic->streams[0]->codec != NULL)?ic->streams[0]->codec->codec_id:0);
        av_log(NULL, AV_LOG_INFO, "4-1, 1: %d", (ic->streams[1] && ic->streams[1]->codec != NULL)?ic->streams[1]->codec->codec_id:0);
    }
 ...
 find_stream_info_err:
    ...
    if(ic->pb)
        av_log(NULL, AV_LOG_DEBUG, "File position after avformat_find_stream_info() is %"PRId64"\n", avio_tell(ic->pb));
    return ret;
}
