...
static void rtmp_metdata_cb(RTMP *r , AMFObject* md){
 ...
 */
static int rtmp_open2(URLContext *s, const char *uri, int flags, AVDictionary **options)
{
    LibRTMPContext *ctx = s->priv_data;
    RTMP *r = &ctx->rtmp;
    int rc = 0, level;
...
//    RTMP_LogSetCallback(rtmp_log);
    ...
    RTMP_Init(r);
    AVDictionaryEntry *t = av_dict_get(*options, "holder", NULL, 0);
    if(NULL != t && NULL != t->value){
    	av_log(NULL, AV_LOG_INFO, "call RTMP_RegisterCB in rtmp_open2\n");
    	beseyeCBFuncHolder* holder = (beseyeCBFuncHolder*)t->value;
        av_log(NULL, AV_LOG_DEBUG, "call RTMP_RegisterCB, holder:%x, rtmpCallback:%x, rtmpStatusCallback:%x, rtmpErrorCallback:%x, userData:%x\n", holder, holder->rtmpCallback, holder->rtmpStatusCallback, holder->rtmpErrorCallback, holder->userData);
...
{
    return rtmp_open2(s, uri, flags, NULL);
}
