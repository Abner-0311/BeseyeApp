...
{
    //av_log(NULL, AV_LOG_DEBUG, "av_dict_set(), (%s, %s, 0x%x)\n", key, value, flags);
    AVDictionary      *m = *pm;
    ...
    if(tag) {
        av_log(NULL, AV_LOG_DEBUG, "av_dict_set(), tag is not null\n");
        ...
        *tag = m->elems[--m->count];
        ...
        av_log(NULL, AV_LOG_DEBUG, "av_dict_set(), tag is null\n");
    ...
    if (value) {
        ...
        if ((flags & AV_DICT_DONT_STRDUP_VAL) || 0 == strcmp(key, "holder")) {
            ...
            av_log(NULL, AV_LOG_DEBUG, "av_dict_set(), duplicate value\n");
