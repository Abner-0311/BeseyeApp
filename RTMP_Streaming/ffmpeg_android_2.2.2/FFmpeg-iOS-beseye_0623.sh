...
EXTRA_CONFIG="--arch=${ARCH} --disable-asm --enable-cross-compile --target-os=darwin"
...
if [ "${ARCH}" == "arm64" ];
then
EXTRA_CONFIG="--arch=${ARCH} --target-os=darwin --enable-cross-compile --disable-armv5te"
else
...
cd ${INTERDIR}
xcrun -sdk iphoneos lipo -output universal/lib/$file  -create -arch armv7 armv7/lib/$file -arch armv7s armv7s/lib/$file -arch i386 i386/lib/$file -arch arm64 arm64/lib/$file -arch x86_64 x86_64/lib/$file
