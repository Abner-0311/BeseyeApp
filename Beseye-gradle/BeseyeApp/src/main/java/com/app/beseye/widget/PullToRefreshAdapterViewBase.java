...
public abstract class PullToRefreshAdapterViewBase<T extends AbsListView> extends PullToRefreshBase<T> implements
	...
	public PullToRefreshAdapterViewBase(Context context, LvExtendedMode mode) {
		super(context, mode);
		mRefreshableView.setOnScrollListener(this);
	}
	 ...
	 * {@link LvExtendedMode#PULL_DOWN_TO_REFRESH}. The default value is
	...
	public final void setEmptyView(View newEmptyView, int iWidth, int iHeight) {
		...
		if (null != newEmptyView) {
			...
			mRefreshableViewHolder.addView(newEmptyView, iWidth, iHeight);
	 ...
	 * a state where a Pull-to-Refresh can happen. An example of this state is
	 * when the Adapter View is scrolled to the top and the mode is set to
	 * {@link LvExtendedMode#PULL_DOWN_TO_REFRESH}
	 * 
	 * @param showIndicator
	...
	protected void handleStyledAttributes(TypedArray a) {
		...
		mShowIndicator = true;//a.getBoolean(R.styleable.PullToRefresh_ptrShowIndicator, DEFAULT_SHOW_INDICATOR);
	}
	...
	private void addIndicatorViews() {
		LvExtendedMode mode = getMode();

		if (mode.canPullDownUpdate() && null == mIndicatorIvTop) {
			// If the mode can pull down, and we don't have one set already
			mIndicatorIvTop = new IndicatorLayout(getContext(), LvExtendedMode.PULL_DOWN_TO_REFRESH);
		...
		} else if (!mode.canPullDownUpdate() && null != mIndicatorIvTop) {
			// If we can't pull down, but have a View then remove it
			mRefreshableViewHolder.removeView(mIndicatorIvTop);
			mIndicatorIvTop = null;
		}

		if (mode.canPullUpLoadMore() && null == mIndicatorIvBottom) {
			// If the mode can pull down, and we don't have one set already
			mIndicatorIvBottom = new IndicatorLayout(getContext(), LvExtendedMode.PULL_UP_TO_REFRESH);
			FrameLayout.LayoutParams params = new FrameLayout.LayoutParams(ViewGroup.LayoutParams.WRAP_CONTENT,
					ViewGroup.LayoutParams.WRAP_CONTENT);
			params.rightMargin = getResources().getDimensionPixelSize(R.dimen.indicator_right_padding);
		...
		} else if (!mode.canPullUpLoadMore() && null != mIndicatorIvBottom) {
			// If we can't pull down, but have a View then remove it
