...
int GetAudioBufCGI(const char* command, char* data, const char* session, void(* writeBufCB)(unsigned char*, int))
{
	...
	sprintf(url, "%s?session=%s",HOST_NAME_AUDIO, session);
	LOGE("url:%s\n", url);
	memset(data, 0, BUF_SIZE);
    // get the current settings
    res = getdataExt(url, "receiveRaw", session, writeBufCB);
    if (res != 0) {
        // error occurred
        fprintf(stderr, "CURL error: %d\n", res);
        return res;
...
int getdataExt(char* url, const char* command, char* session, void(* writeBufCB)(unsigned char*, int))
{
  CURL *curl;
  CURLcode res;
  ...
  sprintf(request, "GET /cgi/audio/%s.cgi?session=%s&httptype=singlepart HTTP/1.0\r\nHost: 192.168.2.2\r\n\r\n", command, session);
